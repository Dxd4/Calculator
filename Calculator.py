...
def main():
	Label = tk.Label(root,font=("Calibri", 20),height=1,padx=10,pady=10,relief="flat",text="")
	Label.grid(column=0,row=0,columnspan=4)
		
	for x in range(size+1):
		for y in range(size):
			buttons.append(tk.Button(root,text=f"{buttons_text[size*x+y]}",font=("Calibri", 20),background="#bbbbbb",relief="groove",height=2,width=4,command=partial(calculate,buttons_text[size*x+y],Label)))
